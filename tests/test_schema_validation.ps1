...
$schemaMatches = [regex]::Matches($schemaRaw, '"(t:[^"]+)"')
Write-Host "Total translation references in settings_schema.json: $($schemaMatches.Count)"
$missingCount = 0
$seen = @{}
...
foreach ($sf in $sectionFiles) {
    ...
    if ($content -match '\{%\s*schema\s*%\}([\s\S]*?)\{%\s*endschema\s*%\}') {
        ...
        $secTotalCount += $secMatches.Count
...
Write-Host "Total section schema translation references verified: $secTotalCount"
